...
| Option | Trade-off |
| --- | --- |
| `devbook-derived`, an L1 extension on the `devbook-collaboration` template, taking `Update-DevbookIndex.ps1`, `devbook-meta-nightly.yml`, `devbook-derived-artifacts.md` with its `**/_meta/**` rule entry, the `_meta/` paragraphs of the `AGENTS.md` section, and `devbook-check` step 6 | What the layer model already describes. devbook keeps `tools/devbook-meta/`, `devbook-meta.yml`, and the `--check` gate, so a devbook-only repository still validates its own schema. L1 invokes `.devbook/tools/devbook-meta/build.mjs` at the path devbook already materializes it to, so no module crosses a plugin boundary. Costs a migration: assets leaving the materialize table have to be pulled from repositories that do not adopt the new plugin |

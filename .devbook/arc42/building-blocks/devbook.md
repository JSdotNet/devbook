...
| [delivery](delivery.md#dependencies) | **Undeclared** — see [debt record 4](../tdr/4-delivery-depends-on-devbook.md) | Five folder flows name the folders, restate three schema rules, and run this block's generator at the path the install writes it to | None declared, on either side | Folder names, the generator's payload path, and the contract version — none of which it pins. |
...
without demoting all twenty-four of its skills wherever this block is absent, and cannot be

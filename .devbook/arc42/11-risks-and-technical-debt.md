...
| [8. The domain rule is not exercised here](tdr/8-the-domain-rule-is-not-exercised-here.md) | 2026-09-21 | Low | identified | `devbook-domain.md` is the largest rule devbook ships and the four `type`-driven converter kinds run on it; this repository dropped `domain/` for one building block per plugin under `arc42/`, so its own devbook no longer keeps that rule and only a consumer's does. |

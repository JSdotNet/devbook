...
| --- | --- | --- | --- | --- |
| [1. The body budgets are unenforced](tdr/1-body-budgets-unenforced.md) | 2026-09-04 | Low | in-progress | AGENTS.md states three body budgets and `check-assets --budgets` reports against them; 22 of 77 budgeted assets meet them, and the kinds that are long by nature are exempted by decision rather than trimmed. |
| [2. fleet names the Claude CLI directly](tdr/2-fleet-names-the-cli-directly.md) | 2026-09-03 | Medium | resolved | `fleet` named a host's CLI although no asset here may name one. Resolved 2026-09-21 by deleting the plugin: the issue sweep runs in the schedule's own session and spawns nothing. |
...
| [6. sync-specs borrows a name OpenSpec uses for something else](tdr/6-sync-specs-borrows-a-name-openspec-uses-for-something-else.md) | 2026-09-15 | Low | resolved | OpenSpec's `sync-specs` merges spec deltas a proposal wrote and never reads source; devbook's reads source and writes the chapter, the direction OpenSpec has no skill for. Resolved 2026-09-22 on the trigger the record named — the next change carrying a rename anyway — by taking the protocol's own word: the skill is `capture-specs`, and the two borrowed names left mean here what they mean there. |
...
| [8. The domain rule is not exercised here](tdr/8-the-domain-rule-is-not-exercised-here.md) | 2026-09-21 | Medium | identified | `devbook-domain.md` is the largest rule devbook ships and the four `type`-driven converter kinds run on it; this repository dropped `domain/` for one building block per plugin under `arc42/`, so its own devbook no longer keeps that rule and only a consumer's does. Contract 14 has since added `requirements.md` and `invariants.md`, two chapter kinds with a template each that nothing here has ever written, and contract 17 moved invariants into a subpage of each domain page. |

...
| [5. The derived index is not optional](tdr/5-derived-index-is-not-optional.md) | 2026-09-08 | Medium | resolved | devbook shipped the derived `_meta/` index as foundation although nothing here read it. Resolved on 2026-09-17 by [record 77](adr/77-the-tooling-is-devbook-deriveds.md): the writer, the check, and the canvas are `devbook-derived`'s, an L1 a repository enables or does not. |

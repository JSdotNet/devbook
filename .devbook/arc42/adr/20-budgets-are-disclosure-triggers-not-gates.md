...
| `assets/spec-kinds/<kind>.md`, read by `sync-specs`, `propose-change`, and `verify-change` — the `to-spec-*` and `from-spec-*` skills until [record 67](67-the-converters-are-three-skills-named-after-openspec.md) | Each carries the full mapping between one chapter kind and code, and a mapping stated by half is wrong. |

...
| [Install](install.md) | A plugin reaches a repository only through its `init` and is kept current by its `update`, one way, hash-tracked; `validate` checks the corpus and `devbook-config:doctor` the installation; only `devbook` carries a ledger; this repository stamps itself and materializes nothing; component pairs are hidden from the menu behind `devbook-config`. | 2026-09-25 |
...
| [Releases](releases.md) | Every plugin `1.7.0`, moved together; a contract bump is a minor release and ships a migration when one is owed, which lives for one major; the marketplace name is a frozen per-machine key. | 2026-09-25 |

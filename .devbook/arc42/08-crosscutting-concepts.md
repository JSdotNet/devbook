...
| The decision rungs — `approved` and `accepted`, each with a signer, a day, and an optional content fingerprint | [devbook](building-blocks/devbook.md) | [devbook-collaboration](building-blocks/devbook-collaboration.md)'s approval and acceptance gates, and anyone writing a rung by hand | Two `status` values and six optional fields in `rules/devbook-chapter-metadata.md`; the fingerprint is computed by `tools/devbook-meta/chapter-hash.mjs` |

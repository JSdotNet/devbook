...
| [Devbook](../devbook/dependencies.md) | Conformist, read-only | Reads which folders are adopted under `.devbook/`, and invokes `devbook:install` during setup and update | The folder layout, and the install skill's name | It is named for the folder it writes into, not for a plugin it needs. One it names but cannot find is reported as not installed. |
| [Devbook Derived](../devbook-derived/dependencies.md), [Delivery Schedule](../delivery-schedule/dependencies.md) | Conformist, read-only | Reads their stamps and invokes their install skills during a fan-out | The stamp shape and each install skill's name | Every component's stamp stays with the component. This context decides *whether* an install runs and never what it does. |

...
| [Devbook](../devbook/dependencies.md) | Separate Ways | One catalog entry names `prose-check`, and two name `devbook-derived:check` and `devbook-derived:tech-update` as targets | The skill names alone | Naming is not depending: a trigger whose target plugin the repository has not enabled is reported and skipped, never scheduled. |

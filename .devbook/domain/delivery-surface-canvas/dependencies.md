...
| [Delivery](../delivery/dependencies.md) | Conformist to a Published Language | Implements the render group's two operation names, and nothing else | `resources/surface-contract.md`: `delivery.surface.render@1` | The contract matches operation names rather than a transport, which is exactly what lets a canvas satisfy it without being an MCP server. |
...
- **It answers one group and is substitutable within it.** On the other host, the
  [dashboard](../delivery-surface-dashboard/dependencies.md) answers the same group — which is

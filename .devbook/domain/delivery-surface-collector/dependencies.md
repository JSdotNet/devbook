...
| [Delivery Schedule](../delivery-schedule/dependencies.md) | Customer-Supplier, this context supplying | Lifecycle calls from an unattended cloud session | The same contract | That a run recorded with nobody watching is readable afterwards — which is the case this implementation exists for. |
| [Fleet](../fleet/dependencies.md) | Customer-Supplier, this context supplying | Lifecycle calls from a worker session | The same contract | The same, per worker. Its own result files remain the source of truth for the sweep. |

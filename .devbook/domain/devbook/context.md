...
| [Devbook Procedures](../devbook-procedures/context.md#dependencies) | Customer-Supplier, declared | Follows this context's reconcile protocol — the stamp's two shared fields, the hash rules, the plan-before-write phase — and stamps `components.devbook-procedures` beside this context's entry | `assets/reconcile-protocol.md` under **The stamp** | That the protocol and the stamp keep their shape; it reads no chapter and runs no check. |

...
No topic for `.devbook/**` is authored here. The rules for the devbook folders are the
delivered ones above, and the devbook section of `AGENTS.md` — rendered by `devbook:update`
between its markers — points at them, so a second authored copy would be the one this

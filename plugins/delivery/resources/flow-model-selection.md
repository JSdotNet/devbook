...
A stage may list entries from more than one category (for example `flow-code`'s "Reproduction &
Root Cause", naming the `implement` service and the `app.start` service). Resolve the model

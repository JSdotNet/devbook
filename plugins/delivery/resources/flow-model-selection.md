...
| **Spec Verification** | Spec Verification | the `verify` service | `opus` | A verdict on whether the change set is what was agreed is review judgment: `aligned` called on drift ships a wrong pull request, so it takes the strongest reasoning. |

...
check("an unlabelled kind keeps its id", await titleFor("Pin Aspire 9", [edit(".tech/backend.md")], capitalized), "tech — Pin Aspire 9");

const grouped = { labels: { devbook: "devbook", code: null } };
check("the devbook group covers every folder", await titleFor("Runtime view refresh", [write(".arc42/06-runtime-view.md")], grouped), "devbook — Runtime view refresh");
check("a domain chapter still names its context under the group", await titleFor("Add Fulfilment aggregate", [write(".domain/order-management/domain.md")], grouped), "devbook:order-management — Add Fulfilment aggregate");
check("folders sharing a label tally as one", await titleFor("Spec sweep", [write(".arc42/01-intro.md"), write(".domain/billing/domain.md"), edit("src/A.cs"), edit("src/B.cs")], grouped), "devbook:billing — Spec sweep");
check("a folder's own key wins over the group", await titleFor("Dense table tokens", [edit(".design/color-scheme.md")], { labels: { devbook: "devbook", design: "ux" } }), "ux — Dense table tokens");
check("a null label is no rename", await titleFor("Rounding fix", [edit("src/Shipping/Rate.cs")], grouped), null);
check("a null kind still competes for dominance", await titleFor("Mostly code", [write(".domain/billing/domain.md"), edit("src/A.cs"), edit("src/B.cs")], grouped), null);

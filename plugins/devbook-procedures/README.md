...
neither `capture` nor `show`; `estimate` needs neither.

## What it ships

| Part | What it is |
|---|---|
| `assets/skills/<name>.md` | The five seeds: `name`, `description`, and `goal` in the frontmatter, an example-filled procedure below |
...
- **Nothing depends on a procedure.** A caller that names `start`, `capture`, or `estimate`
  and finds it absent does without and says so. `show` composes `start` and `capture` by

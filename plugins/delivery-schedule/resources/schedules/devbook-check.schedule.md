...
Fix what the check reports in the source Markdown and re-run until it exits `0`. If anything
changed, open the pull request titled `chore(devbook): daily check <YYYY-MM-DD>` with the
findings and the fixes in its body.
...
One schedule covers every folder. The checker walks them in one pass and a stale reference

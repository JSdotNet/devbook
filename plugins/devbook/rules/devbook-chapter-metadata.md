...
command to `TEST_RUNNERS` in `.devbook/tools/devbook-meta/metadata.mjs`.
...
`.devbook/tools/devbook-meta/metadata.mjs`, returns
...
`.devbook/tools/devbook-meta/outline.mjs`. Keep that table and the folders'
**Structure** blocks in step with each other.

## Derived metadata index

These metadata blocks are compiled into derived indexes by
`.devbook/tools/devbook-meta/build.mjs` — one pair per devbook folder plus
...
the devbook-meta tooling README (`.devbook/tools/devbook-meta/README.md`) for the output shape.

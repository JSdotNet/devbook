...
Then confirm git tracks it — `git check-ignore -v .devbook/ai/adoption-map.md`. A repo
holding Adobe Illustrator files often carries a `*.ai` rule; `.devbook/ai/` does not
match it, but a file inside named `<something>.ai` would — check before adopting.

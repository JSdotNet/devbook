...
4. **Fan out, in the report's order.** For each `reconcile` row, invoke that component's own
   install skill — `devbook:install`, then `delivery:install`, then
   `delivery-schedule:install` — and let it run its migrations oldest first, overwrite what is
   stale, leave what is customized, and rewrite its own stamp. `devbook-collaboration` has no
   install: enabling it is the whole adoption.

   The order is load-bearing at the end: schedule checks its targets against what the
   repository enables. `delivery:install` re-seeds the `start` and `capture` copies and

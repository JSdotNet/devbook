...
| Unattended | `delivery-schedule` | Work that runs with nobody watching: ten `schedule-*` entry points that pick their own input and run a flow or a review, and seven triggers a repository selects from and syncs into the host's scheduler — its Routines page in Claude Code, its Automations page in the GitHub Copilot app. |

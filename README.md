...
| Unattended | `delivery-schedule` | Work that runs with nobody watching: fifteen `schedule-*` entry points that pick their own input and run a flow, a review, a triage, or a report, and eleven triggers a repository selects from and syncs into the host's scheduler — its Routines page in Claude Code, its Automations page in the GitHub Copilot app. |

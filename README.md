...
| --- | --- | --- |
| Devbook | `devbook`, `devbook-derived`, `devbook-collaboration` | Addressed Markdown chapters under `.devbook/` with parseable `meta` blocks, generated `_meta/` indexes, a reference-graph canvas, converters between chapters and code, and review and approval workflows over them. |
| Delivery | `delivery`, `fleet` | Four `flow-*` procedures that carry a change from request to a validated commit — the code, the devbook folders, the dependencies, the project — and the fan-out lane that works a backlog across sessions. |
...
| Unattended | `delivery-schedule` | Work that runs with nobody watching: fourteen `schedule-*` entry points that pick their own input and run a flow, a review, or a report, and ten triggers a repository selects from and syncs into the host's scheduler — its Routines page in Claude Code, its Automations page in the GitHub Copilot app. |

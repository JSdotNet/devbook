...
| --- | --- | --- |
| Devbook | `devbook`, `devbook-collaboration` | Addressed Markdown chapters under `.devbook/` (or five root dot-folders) with parseable `meta` blocks, a reference-graph canvas, converters between chapters and code, and review and approval workflows over them. |
...
| Unattended | `delivery-schedule` | Work that runs with nobody watching: twelve `schedule-*` entry points that pick their own input and run a flow, a review, or a report, and ten triggers a repository selects from and syncs into the host's scheduler — its Routines page in Claude Code, its Automations page in the GitHub Copilot app. |
